...
//Simport javax.annotation.PreDestroy;
...
public class UserBean implements Serializable {
	...
	public void checkSession() {
		if (user == null)
			return;
		if (getSessionId() != Appmanager.getInstance().getSessionMap()
				.get(user.getId())) {
			logger.info("user " + user.getId() + ": this session ("
					+ getSessionId() + ") did not match previous session: "
					+ Appmanager.getInstance().getSessionMap().get(user.getId()));
			user = null;
		}
	}
	...
	public void setUser(User user) {
		...
		session.setAttribute("webapplication_user", user);
	}
